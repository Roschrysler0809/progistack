...
from .common_dates import is_business_day, adjust_duration_by_workforce
...
from .common_requirements import HOURS_PER_DAY
...
class Project(models.Model):
    ...
    from_etude_chiffrage = fields.Boolean(
        string="Généré depuis Etude et Chiffrage", default=False
    )

    # Determines if project requires devis steps
    requires_devis_steps = fields.Boolean(
        string="Nécessite création de devis",
    ...
    implementation_project_id = fields.Many2one(
        "project.project", string="Projet", readonly=True, copy=False
    )
    show_implementation_project_button = fields.Boolean(
        string="Afficher le bouton du projet d'implémentation",
        compute="_compute_show_implementation_project_button",
    )

    # Messages
    department_tabs_readonly_message = fields.Char(
        compute="_compute_department_tabs_readonly_message", readonly=True
    )

    # Date conditions
    is_weekend_date = fields.Boolean(
        string="Date sur weekend",
        compute="_compute_is_weekend_date",
    ...
    project_type_color = fields.Integer(
        string="Couleur du type de projet",
        compute="_compute_project_type_color",
        store=True,
    ...
    department_type = fields.Selection(
        DEPARTMENT_TYPE_SELECTION, string="Suivi département", tracking=True
    )
    implementation_category = fields.Selection(
        IMPLEMENTATION_CATEGORY_SELECTION,
        string="Catégorie implementation",
        default=False,
        tracking=True,
    )
    etude_chiffrage_category = fields.Selection(
        ETUDE_CHIFFRAGE_CATEGORY_SELECTION,
        string="Catégorie étude/chiffrage",
        tracking=True,
    )
    department_ids = fields.Many2many(
        "project.department", string="Département(s)", required=True
    )
    ...
    available_stages = fields.Selection(
        STAGE_SELECTION,
        string="Étapes disponibles",
        compute="_compute_available_stages",
        help="Liste des étapes disponibles en fonction du type de projet",
    )

    # Devis generation fields
    devis_generated_by = fields.Many2one(
        "res.users", string="Devis généré par", readonly=True, tracking=True
    )
    devis_generated_date = fields.Datetime(
        string="Devis généré le", readonly=True, tracking=True
    )

    # Devis validation fields
    devis_validated_by = fields.Many2one(
        "res.users", string="Devis validé par", readonly=True, tracking=True
    )
    devis_validated_date = fields.Datetime(
        string="Devis validé le", readonly=True, tracking=True
    )

    # Project creation fields
    project_created_by = fields.Many2one(
        "res.users", string="Projet créé par", readonly=True, tracking=True
    )
    project_created_date = fields.Datetime(
        string="Projet créé le", readonly=True, tracking=True
    )

    # Profile lines
    profile_line_ids = fields.One2many(
        "project.profile.line", "project_id", string="Profils"
    )

    # Sale order fields
    sale_order_ids = fields.Many2many(
        "sale.order",
        string="Tous les devis",
        readonly=True,
    ...
    current_sale_order_state = fields.Char(
        string="État du devis actuel",
        compute="_compute_current_sale_order_state",
        store=True,
    )
    all_devis_confirmed = fields.Boolean(
        string="Tous les devis sont confirmés",
    ...
    requirement_line_ids = fields.One2many(
        "project.requirement.line", "project_id", string="Lignes d'exigence"
    )

    # Custom requirement lines for evolution projects
    custom_requirement_line_ids = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
        string="Lignes d'exigence personnalisées",
    )

    # Computed fields for department visibility and requirement lines
    department_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    achat_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    vente_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    finance_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    rh_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
        string="Exigences RH",
        store=True,
    ...
    stock_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    production_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    transverse_requirement_lines = fields.One2many(
        "project.requirement.line",
        "project_id",
    ...
    custom_achat_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    custom_vente_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    custom_finance_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    custom_rh_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    custom_stock_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    custom_production_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    custom_transverse_requirement_lines = fields.One2many(
        "project.custom.requirement.line",
        "project_id",
    ...
    has_achat_department = fields.Boolean(
        string="Département Achat", compute="_compute_has_departments", store=True
    )
    has_vente_department = fields.Boolean(
        string="Département Vente", compute="_compute_has_departments", store=True
    )
    has_finance_department = fields.Boolean(
        string="Département Finance", compute="_compute_has_departments", store=True
    )
    has_rh_department = fields.Boolean(
        string="Département RH", compute="_compute_has_departments", store=True
    )
    has_stock_department = fields.Boolean(
        string="Département Stock", compute="_compute_has_departments", store=True
    )
    has_production_department = fields.Boolean(
        string="Département Production", compute="_compute_has_departments", store=True
    )
    has_transverse_department = fields.Boolean(
        string="Département Transverse", compute="_compute_has_departments", store=True
    )

    # Flag to determine if new requirements are available
    has_new_requirements = fields.Boolean(
        string="Nouvelles exigences disponibles",
        compute="_compute_has_new_requirements",
    )

    # UI visibility flags
    show_insert_requirements_button = fields.Boolean(
        string="Afficher le bouton d'insertion des exigences",
        compute="_compute_show_insert_requirements_button",
    )
    show_insert_requirements_button = fields.Boolean(
        string="Afficher bouton insertion exigences",
        compute="_compute_show_insert_requirements_button",
    )
    show_implementation_project_button = fields.Boolean(
        string="Afficher bouton projet implémentation",
        compute="_compute_show_implementation_project_button",
    )
    show_profiles_tab = fields.Boolean(
        string="Afficher l'onglet Profils", compute="_compute_show_profiles_tab"
    )
    show_requirements_tab = fields.Boolean(
        string="Afficher l'onglet Exigences", compute="_compute_show_requirements_tab"
    )
    show_custom_requirements_tab = fields.Boolean(
        string="Afficher l'onglet Exigences personnalisées",
        compute="_compute_show_custom_requirements_tab",
    )
    show_generate_tasks_button = fields.Boolean(
        string="Afficher bouton génération tâches directe",
        compute="_compute_show_generate_tasks_button",
    )
    is_department_type_readonly = fields.Boolean(
        string="Est-ce que le type de département est en lecture seule",
        compute="_compute_is_department_type_readonly",
    )

    # Field validation flags
    profiles_required = fields.Boolean(
        string="Profils requis", compute="_compute_profiles_required", store=True
    )
    requirements_required = fields.Boolean(
        string="Exigences requises",
        compute="_compute_requirements_required",
        store=True,
    )
    project_requires_custom_requirements = fields.Boolean(
        string="Nécessite des exigences personnalisées",
    ...
    custom_requirements_required = fields.Boolean(
        string="Exigences personnalisées requises",
        compute="_compute_custom_requirements_required",
        store=True,
        help="Indique si ce projet nécessite des exigences personnalisées",
    )
    regular_requirements_required = fields.Boolean(
        string="Exigences standards requises",
    ...
    profiles_workload_valid = fields.Boolean(
        string="Charge des profils valide",
        compute="_compute_profiles_workload_valid",
        store=False,
    )
    profiles_workload_message = fields.Char(
        string="Message validation charge",
        compute="_compute_profiles_workload_valid",
        store=False,
    )
    profiles_vs_requirements_workload_info = fields.Html(
        string="Info charge profils/exigences",
    ...
    lot_ids = fields.One2many(
        "project.department.lot", "project_id", string="Lots de départements"
    )
    lot_count = fields.Integer(compute="_compute_lot_count", string="Nombre de lots")
    has_lots = fields.Boolean(
        compute="_compute_has_lots", string="A des lots définis", store=True
    )
    has_lots_with_departments = fields.Boolean(
        compute="_compute_has_lots_with_departments",
        string="A des lots avec départements",
        store=True,
    )
    all_departments_assigned = fields.Boolean(
        compute="_compute_all_departments_assigned",
        string="Tous les départements sont assignés",
        store=True,
    )

    # Flag to indicate if there are requirement lines
    has_requirement_lines = fields.Boolean(
        string="A des exigences standards",
        compute="_compute_has_requirement_lines",
        store=True,
    )
    has_custom_requirement_lines = fields.Boolean(
        string="A des exigences personnalisées",
        compute="_compute_has_custom_requirement_lines",
        store=True,
    )

    # Flag to indicate if this is an evolution project
    is_evolution_project = fields.Boolean(
        string="Est un projet d'évolution",
    ...
    def _get_department_domain(self):
        ...
        is_evolution = (
            self.project_type == "implementation"
            and self.implementation_category == "evolution"
        )
    ...
    def _compute_is_evolution_project(self):
        ...
        for project in self:
            project.is_evolution_project = (
                project.project_type == "implementation"
                and project.implementation_category == "evolution"
            )
    ...
    def _compute_is_weekend_date(self):
        ...
        for project in self:
            project.is_weekend_date = project.date_start and not is_business_day(
                project.date_start
            )
    ...
    def _compute_department_tabs_readonly_message(self):
        ...
        for project in self:
            if project.stage == "project" and (
                project.project_type != "etude_chiffrage"
                or project.implementation_project_id
            ):
                project.department_tabs_readonly_message = "Les exigences sont en lecture seule lorsque le projet est en phase 'Projet'"
            elif (
                project.stage == "project"
                and project.project_type == "etude_chiffrage"
                and not project.implementation_project_id
            ):
    ...
    def _compute_has_departments(self):
        ...
        for project in self:
            project.has_achat_department = any(
                d.code == "achat" for d in project.department_ids
            )
            project.has_vente_department = any(
                d.code == "commerciale" for d in project.department_ids
            )
            project.has_finance_department = any(
                d.code == "finance" for d in project.department_ids
            )
            project.has_rh_department = any(
                d.code == "rh" for d in project.department_ids
            )
            project.has_stock_department = any(
                d.code == "stock" for d in project.department_ids
            )
            project.has_production_department = any(
                d.code == "production" for d in project.department_ids
            )
            project.has_transverse_department = any(
                d.code == "transverse" for d in project.department_ids
            )
    ...
    def _compute_available_department_ids(self):
        ...
        for project in self:
            ...
            if (
                ...
                if generic_department:
                    project.available_department_ids = all_departments.filtered(
                        lambda d: d.id != generic_department.id
                    )
                else:
                    project.available_department_ids = all_departments

    def get_diff_lines(self, department_field):
        self.ensure_one()
        all_lines = set(self.requirement_line_ids.ids)
        dept_lines = set(self[department_field].ids)
        
        # Lignes dans requirement_line_ids mais pas dans achat_requirement_line_ids
        lines_only_in_requirement = self.requirement_line_ids.filtered(lambda l: l.id not in dept_lines)
        # Lignes dans achat_requirement_line_ids mais pas dans requirement_line_ids
        lines_only_in_dept = self[department_field].filtered(lambda l: l.id not in all_lines)
        return lines_only_in_dept
    
    @api.depends(
        "department_ids",
        "requirement_line_ids",
        "requirement_line_ids.department_id",
    )
    ...
    @api.depends(
        "department_ids",
        "custom_requirement_line_ids",
        "custom_requirement_line_ids.department_id",
    )
    def _compute_department_custom_requirement_lines(self):
        """Compute custom requirement lines for each department"""
        for record in self:
            # Use filtered() directly to assign department-specific custom requirement lines
            record.custom_achat_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "achat"
                )
            )
            record.custom_vente_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "commerciale"
                )
            )
            record.custom_finance_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "finance"
                )
            )
            record.custom_rh_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "rh"
                )
            )
            record.custom_stock_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "stock"
                )
            )
            record.custom_production_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "production"
                )
            )
            record.custom_transverse_requirement_lines = (
                record.custom_requirement_line_ids.filtered(
                    lambda line: line.department_id.code == "transverse"
                )
            )
    ...
    @api.depends(
        "department_ids", "requirement_line_ids", "requirement_line_ids.requirement_id"
    )
    def _compute_has_new_requirements(self):
        ...
        for project in self:
            ...
            existing_requirement_ids = project.requirement_line_ids.mapped(
                "requirement_id.id"
            )
    ...
    def _process_devis_state_change(self, project, devis):
        ...
        if (
            project.current_sale_order_id
            and devis.id != project.current_sale_order_id.id
        ):
            return
        ...
        # Handle canceled devis
            ...
            if (
                project.current_sale_order_id
                and project.current_sale_order_id.id == devis.id
    ...
    def _compute_show_insert_requirements_button(self):
        ...
        for record in self:
            ...
            if record.stage == "project" and (
                record.project_type != "etude_chiffrage"
                or record.implementation_project_id
            ):
                record.show_insert_requirements_button = False
            # Show when there are new requirements available and not in project stage
            elif record.has_new_requirements and record.stage in [
                "preparation",
                "devis_created",
                "devis_validated",
            ]:
            ...
            elif not record.requirement_line_ids and record.stage in [
                "preparation",
                "devis_created",
                "devis_validated",
            ]:
            ...
            # Special case: Show for etude_chiffrage in project stage without implementation project when there are new requirements
                ...
                record.stage == "project"
                and record.project_type == "etude_chiffrage"
                and not record.implementation_project_id
                and record.has_new_requirements
            ):
                record.show_insert_requirements_button = True
            # Special case: Show for etude_chiffrage in project stage without implementation project when there are no requirements
                ...
                and record.project_type == "etude_chiffrage"
                and not record.implementation_project_id
                and not record.requirement_line_ids
            ):
                record.show_insert_requirements_button = True
            else:
                record.show_insert_requirements_button = False
    ...
    def _compute_show_implementation_project_button(self):
        ...
        for record in self:
            ...
            record.show_implementation_project_button = (
                record.project_type == "etude_chiffrage"
                and record.implementation_project_id
            )
    ...
    def _compute_has_custom_requirement_lines(self):
        for project in self:
            project.has_custom_requirement_lines = bool(
                project.custom_requirement_line_ids
            )
    ...
    def _compute_show_requirements_tab(self):
        ...
        for project in self:
            ...
            if (
                ...
                project.show_requirements_tab = (
                    project._project_shows_requirements_tab()
                )
    ...
    def _compute_custom_requirements_required(self):
        ...
        for project in self:
            ...
            project.custom_requirements_required = (
                project.project_requires_custom_requirements
            )
    ...
    def _compute_regular_requirements_required(self):
        ...
        for project in self:
            ...
            project.regular_requirements_required = (
                not project.project_requires_custom_requirements
            )
    ...
    @api.depends(
        "profile_line_ids",
        "profile_line_ids.workload_days",
        "requirement_line_ids",
        "requirement_line_ids.estimated_work_days",
        "custom_requirement_line_ids",
        "custom_requirement_line_ids.estimated_work_days",
    ...
    def _compute_profiles_workload_valid(self):
        ...
        for project in self:
            ...
            total_profile_workload = sum(
                project.profile_line_ids.mapped("workload_days")
            )

            # Calculate total requirements workload
            total_requirement_workload = sum(
                requirement_lines.mapped("estimated_work_days")
            )
            ...
            if total_profile_workload > total_requirement_workload + 0.01:
                ...
                formatted_profile_load = (
                    int(total_profile_workload)
                    if total_profile_workload.is_integer()
                    else total_profile_workload
                )
                formatted_req_load = (
                    int(total_requirement_workload)
                    if total_requirement_workload.is_integer()
                    else total_requirement_workload
                )
    ...
    def _compute_profiles_vs_requirements_workload_info(self):
        ...
        for project in self:
            ...
            if not requirement_lines:
                project.profiles_vs_requirements_workload_info = (
            ...
            formatted_profile_workload = (
                int(profile_workload)
                if profile_workload.is_integer()
                else f"{profile_workload:.2f}"
            )
            formatted_requirement_workload = (
                int(requirement_workload)
                if requirement_workload.is_integer()
                else f"{requirement_workload:.2f}"
            )
    ...
    def _compute_has_lots_with_departments(self):
        for project in self:
            project.has_lots_with_departments = any(
                lot.department_count > 0 for lot in project.lot_ids
            )

    @api.depends(
        "lot_ids", "lot_ids.department_ids", "department_ids", "is_evolution_project"
    )
    def _compute_all_departments_assigned(self):
        ...
        for project in self:
            ...
            project.all_departments_assigned = all(
                dept in assigned_departments for dept in project.department_ids
            )
    ...
    def _compute_is_department_type_readonly(self):
        ...
        for project in self:
            ...
            readonly_due_to_origin = (
                project.from_crm and not project.from_etude_chiffrage
            )
            readonly_due_to_evolution = (
                project.project_type == "implementation"
                and project.implementation_category == "evolution"
            )

            project.is_department_type_readonly = (
                readonly_due_to_stage
                or readonly_due_to_origin
                or readonly_due_to_evolution
            )
    ...
    def _compute_requires_devis_steps(self):
        ...
        for project in self:
            ...
            if (
                project.project_type == "etude_chiffrage"
                and project.etude_chiffrage_category == "non_billable"
            ):
    ...
    def _compute_show_generate_tasks_button(self):
        ...
        for project in self:
            ...
            show_button = (
                project.stage == "preparation" and not project.requires_devis_steps
            )
            ...
            if (
                show_button
                and project._project_requires_profiles()
                and not project.profile_line_ids
            ):
                show_button = False
            ...
            if (
                show_button
                and project.custom_requirements_required
                and not project.custom_requirement_line_ids
            ):
                show_button = False
            elif (
                show_button
                and project.regular_requirements_required
                and not project.requirement_line_ids
            ):
                show_button = False
        ...
        for project in self:
            if project.stage == "project" and not project.date_start:
                raise models.ValidationError(
                    "La date de début du projet est obligatoire lorsque le projet est généré."
                )
    ...
    def _check_valid_stages(self):
        ...
        for project in self:
            if not project.requires_devis_steps and project.stage in [
                "devis_created",
                "devis_validated",
            ]:
                raise models.ValidationError(
                    "Ce type de projet ne peut pas passer par les étapes de devis."
                )
    ...
    def _onchange_project_start_date(self):
        ...
        if (
            self.date_start and self.date_start.weekday() >= 5
        ):  # 5 = Saturday, 6 = Sunday
            return {
                "warning": {
    ...
    def action_clear_requirement_lines(self):
        ...
        if self.stage == "project" and (
            self.project_type != "etude_chiffrage" or self.implementation_project_id
        ):
            raise models.ValidationError(
                "Vous ne pouvez pas supprimer les exigences d'un projet en phase 'Projet'."
            )
    ...
    def action_insert_missing_requirements(self):
        ...
        if self.stage == "project" and (
            ...
            raise models.ValidationError(
                "Vous ne pouvez pas ajouter d'exigences à un projet en phase 'Projet'."
            )
    ...
    def action_insert_all_requirements(self):
        ...
        requirements = self.env["project.requirement"].search(
        ...
        for requirement in requirements:
            ...
            self.env["project.requirement.line"].with_context(
                skip_auto_subrequirements=False, from_requirements_insertion=True
    ...
    def _calculate_average_daily_rate(self):
        ...
        if total_profiles > 0:
            for profile in self.profile_line_ids:
                total_weighted_rate += (
                    profile.daily_rate * profile.involvement_percentage
                )
    ...
    def action_generate_devis(self):
        ...
        if not self.partner_id:
            raise models.ValidationError(
                "Le projet doit avoir un client pour générer un devis."
            )

        # Check if departments are selected
        if not self.department_ids:
            raise models.ValidationError(
                "Veuillez sélectionner au moins un département pour générer un devis."
            )
        ...
        if self._project_requires_requirements() and not requirement_lines:
            if self.custom_requirements_required:
                ...
                raise models.ValidationError(
                    "Veuillez définir au moins une exigence pour générer un devis."
                )
        ...
        if self.project_type == "etude_chiffrage":
            ...
            if not atelier_product:
                ...
                product_vals = {
                    ...
                    "taxes_id": [
                        (6, 0, [])
                    ],  # Ensure no taxes are applied at product level
                    "supplier_taxes_id": [
                        (6, 0, [])
                    ],  # Ensure no purchase taxes applied
            ...
            exigence_product = Product.search(
                [("name", "=", "Implémentation")], limit=1
            )
            ...
            for department in departments:
                ...
                if self.custom_requirements_required:
                    req_lines = self.custom_requirement_line_ids.filtered(
                        lambda r: r.department_id.id == department.id
                    )
                else:
                    # For other project types, use standard requirements
                    req_lines = self.requirement_line_ids.filtered(
                        lambda r: r.department_id.id == department.id
                    )
                ...
                for req_line in req_lines:
                    ...
                    if self.custom_requirements_required:
                        ...
                        line_description = (
                            req_line.description
                            if req_line.description
                            else req_line.requirement_id.name
                        )
        ...
        vals = {
            "sale_order_ids": [
                (4, created_order.id, 0)
            ],  # Append the new devis to existing ones
    ...
    def action_cancel_devis(self):
        ...
        if current_devis.state in ["draft", "sent", "sale"]:
            try:
                ...
                raise models.ValidationError(
                    f"Erreur lors de l'annulation du devis: {str(e)}"
                )
    ...
    def action_return_to_preparation(self):
        ...
        if self.stage != "devis_created":
            raise models.ValidationError(
                "Seuls les projets en état 'Devis créé' peuvent être remis en préparation."
            )
    ...
    def action_validate_devis(self):
        ...
        if not self.current_sale_order_id:
            raise models.ValidationError(
                "Aucun devis actif à valider. Veuillez d'abord générer un devis."
            )
        ...
        try:
            ...
            raise models.ValidationError(
                f"Erreur lors de la confirmation du devis {current_devis.name}: {str(e)}"
            )
        ...
        if current_devis.state != "sale":
            raise models.ValidationError(
                f"Le devis {current_devis.name} n'a pas pu être confirmé correctement."
            )
    ...
    def action_create_implementation_project(self):
        ...
        if not self.date_start:
            raise models.ValidationError(
                "La date de début du projet est obligatoire pour générer le projet."
            )
        ...
        if not self.profile_line_ids and self._project_requires_profiles():
            raise models.ValidationError(
                "Il doit y avoir au moins un profil défini pour générer le projet."
            )
        ...
        for req_line in self.requirement_line_ids:
            ...
            new_req_line = (
                self.env["project.requirement.line"]
                .with_context(skip_auto_subrequirements=True)
                .create(
                    {
    ...
    def action_create_lot(self):
        ...
        next_lot_number = DepartmentLot.with_context(
            default_project_id=self.id
        )._get_next_lot_number()
    ...
    def action_add_custom_requirement_line(self):
        ...
        ctx = {
            "default_project_id": self.id,
            "default_department_id": default_department
            and default_department.id
            or False,
            "hide_project": True,
        ...
        return {
            ...
            "view_id": self.env.ref(
                "project_requirement.view_project_custom_requirement_line_form"
            ).id,
            "target": "new",
            "context": ctx,
    ...
    def action_create_tasks(self):
        ...
        if not self.date_start:
            raise models.ValidationError(
                "La date de début du projet est obligatoire pour générer les tâches."
            )
        ...
        if not self.profile_line_ids and self._project_requires_profiles():
            raise models.ValidationError(
                "Il doit y avoir au moins un profil défini pour générer les tâches."
            )
    ...
    def _apply_default_task_types(self):
        ...
        for stage_xml_id in stages:
            stage = self.env.ref(
                f"project_requirement.{stage_xml_id}", raise_if_not_found=True
            )
    ...
    def _create_etude_chiffrage_tasks(self):
        ...
        if not self.type_ids:
            raise models.ValidationError(
                "Aucune étape de tâche n'est définie pour ce projet."
            )
        ...
        if tasks:
            ...
            action = (
                ...
                .sudo()
                ._for_xml_id("project.action_view_task")
            )
            action.update(
                {
                    "domain": [("id", "in", tasks)],
    ...
    def _create_task_for_requirement(
        self, req_line, estimated_work_days, is_custom_requirement=False
    ):
        """
        ...
        task_vals = {
            ...
            "allocated_hours": estimated_work_days
            * HOURS_PER_DAY,  # Convert days to hours
            "user_ids": [(5, 0, 0)],  # Clear all assigned users
        }
        ...
        if is_custom_requirement:
            department_name = (
                req_line.department_id.name if req_line.department_id else ""
            )
            task_vals.update(
                {
    ...
    def _create_task_for_custom_requirement(self, custom_req_line, estimated_work_days):
        ...
        return self._create_task_for_requirement(
            custom_req_line, estimated_work_days, is_custom_requirement=True
        )
    ...
    def _reorder_requirements_after_save(self):
        ...
        for project in self:
            if project.requirement_line_ids:
                ...
                self.env["project.requirement.line"]._reorder_project_requirements(
                    project.id
                )
        return True
    ...
    def write(self, vals):
        ...
        if "project_type" in vals:
            ...
            for record in self:
                ...
                if (
                    record.project_type == "implementation"
                    and record.implementation_category == "evolution"
                    and vals["project_type"] != "implementation"
                ):
                    if (
                        generic_department
                        and generic_department in record.department_ids
                    ):
                        # Only modify departments if not explicitly set by user
                        if "department_ids" not in vals:
                            # Remove the generic department
                            new_departments = record.department_ids.filtered(
                                lambda d: d.id != generic_department.id
                            )
                            if (
                                not new_departments
                            ):  # Ensure we don't end up with empty departments
        ...
        if (
            "implementation_category" in vals
            and vals["implementation_category"] == "evolution"
        ):
            if generic_department:
                ...
                for record in self:
                    if record.requirement_line_ids:
                        vals["requirement_line_ids"] = [
                            (5, 0, 0)
                        ]  # Clear all standard requirement lines
            ...
            for record in self:
                ...
                if record.implementation_category == "evolution":
                    ...
                    if record.custom_requirement_line_ids:
                        vals["custom_requirement_line_ids"] = [
                            (5, 0, 0)
                        ]  # Clear all custom requirement lines
        ...
        if "implementation_category" in vals and vals.get("implementation_category"):
            for record in self:
                if (
                    record.project_type == "implementation"
                    or vals.get("project_type") == "implementation"
                ):
                ...
                if project.lot_ids and generic_department:
                    for lot in project.lot_ids:
                        ...
                        if generic_department in lot.department_ids:
                            ...
                            new_lot_departments = lot.department_ids.filtered(
                                lambda d: d.id != generic_department.id
                            )
        ...
        dept_fields = [ fname for fname in self._fields if fname.endswith('_requirement_line_ids') and not fname.startswith('custom_') and fname != 'requirement_line_ids']
        print("dept_fields",dept_fields)
        # for field in dept_fields:
        #     if field in vals:
        #         only_in_dept = self.get_diff_lines(field)
        #         if only_in_dept:
        #             self.requirement_line_ids = vals[field]
        
        print("WRITE project: vals=", vals, "requirement_line_ids=", self.requirement_line_ids.ids)
    ...
    def _remove_requirement_lines_for_removed_departments(self):
        ...
        for project in self:
            ...
            print(f"[REMOVE REQ LINES] Départements projet: {department_ids}")
            ...
            print(f"[REMOVE REQ LINES] Lignes supprimées: {to_remove.ids}")
            if to_remove:
                to_remove.with_context(
                    skip_auto_subrequirements=True, from_requirements_insertion=True
                ).unlink()
